...
def proof_constraints(proof, context):
    """
    Returns the facts and requirements generated by the proof.
    :param proof: the result from Parser.parse()
    :param context: the dict of line numbers and their facts
    :return: the context with the new facts generated by the proof, or None if the proof is invalid
    """
    if len(proof) == 2:
        for sub_proof in proof[1]:
            new_context = proof_constraints(sub_proof, context)
            if new_context is None:
                return None
            context |= new_context
        # TODO add the main proof
        return context
    else:
        # TODO check validity
        return context | {proof[0]: proof[1]}
